...
class LogWatcher:
        ...
        try:
            with path.open('r', encoding='utf-8') as f:
                if path.suffix.lower() == '.json':
                    ...
                    print(f"{Fore.RED}Unsupported config format: {path.suffix}, expected .json{Style.RESET_ALL}")
                    return default_config
    ...
    async def tail_log(self, log_info):
        ...
        print(f"{Fore.GREEN}Started watching: {log_name} ({file_path}){Style.RESET_ALL}")
        ...
        for error_type, pattern in self.error_patterns.items():
            ...
            if match:
                ...
                highlighted_line = pattern.sub(
                    lambda m: f"{color}{m.group(0)}{Style.RESET_ALL}", 
                    line
                )
                print(f"[{timestamp}] {Fore.GREEN}{log_name}{Style.RESET_ALL} "
    ...
    async def start(self):
        ...
        self.tasks = [
            asyncio.create_task(self.tail_log(log))
...
def parse_arguments():
    ...
    parser.add_argument('-c', '--config', default=default_config, help='Path to configuration file (YAML or JSON)')
...
async def main():
    ...
    if args.logs and not args.config:
        config = {
            'logs': [{'path': path, 'name': os.path.basename(path)} for path in args.logs]
        }
        watcher = LogWatcher()
